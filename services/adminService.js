...
const AdminService = {
    createAdmin: async (adminData) => {
        return new Promise((resolve, reject) => {
            ...
            const { username, password, email, app_id, app_certificate, channel_name, token_id, adminlimits, name, customerId, customerSecret } = adminData;
            ...
            db.query(checkSql, [username], (err, results) => {
                ...
                const insertSql = `INSERT INTO admin (username, password, email, app_id, app_certificate, channel_name, token_id, adminlimits, name, customerId, customerSecret) 
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?)`;

                db.query(insertSql, [username, password, email, app_id, app_certificate, channel_name, token_id, adminlimits, name, customerId, customerSecret], (err, result) => {
                    if (err) {
                        console.error("Database Error:", err);
                        return reject(err);
                    }
    ...
    getAdminDropdown: async () => {
        return new Promise((resolve, reject) => {
            const sql = `SELECT id, name FROM admin`;
